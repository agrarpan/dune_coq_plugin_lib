...
let apply_eq (app : eq_app) : types =
  Apputils.mkAppl (eq, [app.at_type; app.trm1; app.trm2])

(*
 * Deconstruct an eq type
 *)
let dest_eq (trm : types) : eq_app =
  let [at_type; trm1; trm2] = Apputils.unfold_args trm in
  { at_type; trm1; trm2 }
...
let apply_eq_sym (app : eq_sym_app) : types =
  ...
  Apputils.mkAppl (eq_sym, [eq_typ.at_type; eq_typ.trm1; eq_typ.trm2; app.eq_proof])

(*
 * Deconstruct an eq type
 *)
let dest_eq_sym (trm : types) : eq_sym_app =
  let [at_type; trm1; trm2; eq_proof] = Apputils.unfold_args trm in
...
let apply_eq_ind (app : eq_ind_app) : types =
  Apputils.mkAppl (eq_ind, [app.at_type; app.trm1; app.p; app.b; app.trm2; app.h])

(* 
 * Deconstruct an eq_ind
 *)
let dest_eq_ind (trm : types) : eq_ind_app =
  let [at_type; trm1; p; b; trm2; h] = Apputils.unfold_args trm in
...
let apply_eq_refl (app : eq_refl_app) : types =
  Apputils.mkAppl (eq_refl, [app.typ; app.trm])

(* 
 * Deconstruct an eq_refl
 *)
let dest_eq_refl (trm : types) : eq_refl_app =
  let [typ; trm] = Apputils.unfold_args trm in
  { typ; trm }
...
let rewr_app f app =
  ...
  Apputils.mkAppl (f, args @ Array.to_list app.params)
