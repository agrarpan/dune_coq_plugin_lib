...
let pack_existT (app : existT_app) : types =
  Apputils.mkAppl (existT, [app.index_type; app.packer; app.index; app.unpacked])

(*
 * Deconstruct an existT term
 *)
let dest_existT (trm : types) : existT_app =
  let [index_type; packer; index; unpacked] = Apputils.unfold_args trm in
...
let pack_sigT (app : sigT_app) =
  Apputils.mkAppl (sigT, [app.index_type; app.packer])

(*
 * Deconsruct a sigT type from a type
 *)
let dest_sigT (typ : types) =
  let [index_type; packer] = Apputils.unfold_args typ in
  { index_type; packer }
...
let elim_sigT (app : sigT_elim) =
  ...
  let arg = app.arg in
  Apputils.mkAppl (sigT_rect, [index_type; packer; packed_type; unpacked; arg])

(*
 * Deconstruct an application of sigT_rect
 *)
let dest_sigT_elim (trm : types) =
  let [index_type; packer; packed_type; unpacked; arg] = Apputils.unfold_args trm in
...
let project_index (app : sigT_app) trm =
  Apputils.mkAppl (projT1, [app.index_type; app.packer; trm])

(*
 * Right projection of a sigma type
 *)
let project_value (app : sigT_app) trm =
  Apputils.mkAppl (projT2, [app.index_type; app.packer; trm])
