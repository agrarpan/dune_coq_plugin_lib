...
let apply_pair (app : pair_app) =
  Apputils.mkAppl (pair, [app.typ1; app.typ2; app.trm1; app.trm2])

(*
 * Deconsruct a sigT type from a type
 *)
let dest_pair (trm : constr) =
  let [typ1; typ2; trm1; trm2] = Apputils.unfold_args trm in
  { typ1; typ2; trm1; trm2 }
...
let apply_prod (app : prod_app) : types =
  Apputils.mkAppl (prod, [app.typ1; app.typ2])

(*
 * Deconstruct a prod
 *)
let dest_prod (trm : types) : prod_app =
  let [typ1; typ2] = Apputils.unfold_args trm in
  { typ1; typ2 }
...
let elim_prod (app : prod_elim) =
  ...
  let arg = app.arg in
  Apputils.mkAppl (prod_rect, [typ1; typ2; p; proof; arg])

(*
 * Deconstruct an application of prod
 *)
let dest_prod_elim (trm : constr) =
  let [typ1; typ2; p; proof; arg] = Apputils.unfold_args trm in
...
let prod_fst (app : prod_app) trm =
  Apputils.mkAppl (fst, [app.typ1; app.typ2; trm])

(*
 * Second projection of a prod
 *)
let prod_snd (app : prod_app) trm =
  Apputils.mkAppl (snd, [app.typ1; app.typ2; trm])
