...
let infer_type env sigma term =
  ...
  let typ = Typing.type_of ~refresh:true env sigma eterm in
  sigma, EConstr.to_constr (fst typ) (snd typ)

(* Safely infer the sort of a type, updating the evar map *)
let infer_sort env sigma term =
  let eterm = EConstr.of_constr term in
  let sort = Typing.sort_of env sigma eterm in
  (fst sort), Sorts.family (snd sort)
